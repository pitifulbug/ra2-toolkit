...
{
    internal const int AutoBuildMaximumRadius = 20;
    ...
    private void ToggleInstantBuild()
    {
        ...
        Console.WriteLine("[瞬间建造已开启] 将持续推进当前玩家工厂中的生产项目。");
    }
    ...
    private void MaintainInstantBuild()
    {
        ...
        WithSuspendedProcess(() =>
        {
            ...
            if (house == 0)
                return;
            var frame = ReadInt32(CurrentFrame);
            foreach (var factory in ReadPointerTable(FactoryArray, 256))
            {
                if (ReadUInt32(factory + FactoryOwnerOffset) != house ||
                    ReadUInt32(factory + FactoryObjectOffset) == 0 ||
                    ReadByte(factory + FactoryOnHoldOffset) != 0 ||
                    ReadByte(factory + FactoryIsSuspendedOffset) != 0 ||
                    ReadInt32(factory + FactoryProductionValueOffset) >= 54)
                    continue;
                AdvanceFactoryProductionCore(factory, frame);
            }
        });
    }
    ...
    private void DisableInstantBuild()
    {
        ...
        Console.WriteLine("[瞬间建造已关闭] 后续生产已交还游戏正常处理。");
    }
    ...
    {
        if (!TryEnqueueEvent(CreateProductionEvent(0x0E, state.TypeIndex)))
        {
            state.Candidates.Enqueue(state.CurrentCandidate);
    ...
    private void MaintainAutoBuildProduction(AutoBuildState state, DateTime now)
    {
        ...
        if (factory != 0)
        {
            ...
            if (ReadByte(factory + FactoryOnHoldOffset) == 0)
                AdvanceFactoryProduction(factory, state.House);
        }
    ...
    private void SendAutoBuildPlacement(AutoBuildState state, DateTime now)
    {
        ...
        if (!TryEnqueueEvent(CreatePlaceEvent(state.TypeIndex, state.CurrentCandidate)))
        {
            state.Candidates.Enqueue(state.CurrentCandidate);
            state.Phase = AutoBuildPhase.FindingPlacement;
            state.NextActionAt = now + TimeSpan.FromMilliseconds(100);
    ...
    private Queue<(short X, short Y)> CreateAutoBuildCandidates((int X, int Y) center)
    {
        ...
        return new Queue<(short X, short Y)>(
            CreateAutoBuildCandidateCells(center, bounds, AutoBuildMaximumRadius));
    }

    internal static IReadOnlyList<(short X, short Y)> CreateAutoBuildCandidateCells(
        (int X, int Y) center,
        (long Left, long Top, long Right, long Bottom) bounds,
        int maximumRadius)
    {
        if (bounds.Right < bounds.Left || bounds.Bottom < bounds.Top)
            return [];

        var radiusLimit = Math.Clamp(maximumRadius, 0, AutoBuildMaximumRadius);
        var diameter = radiusLimit * 2 + 1;
        var candidates = new List<(short X, short Y)>(diameter * diameter);
        ...
        uint typePointer, uint house, (short X, short Y) cell)
    {
        ...
        try
        {
            ...
            PublishCodePatch(LogicUpdate, LogicUpdateOriginalBytes, installedJump,
                "建筑位置检查主循环", ref hookMayBePublished);
        ...
        {
            if (hookMayBePublished && installedJump is not null)
            {
                try
                {
                    _ = RestoreOwnedCodePatch(LogicUpdate, LogicUpdateOriginalBytes,
                        installedJump);
                }
    ...
    private void AdvanceFactoryProduction(uint factory, uint house)
    {
        WithSuspendedProcess(() =>
        {
            if (ReadUInt32(CurrentPlayer) != house ||
                !VectorContains(FactoryArray, factory) ||
                ReadUInt32(factory + FactoryOwnerOffset) != house ||
                ReadUInt32(factory + FactoryObjectOffset) == 0 ||
                ReadByte(factory + FactoryOnHoldOffset) != 0 ||
                ReadByte(factory + FactoryIsSuspendedOffset) != 0 ||
                ReadInt32(factory + FactoryProductionValueOffset) >= 54)
                return;
            AdvanceFactoryProductionCore(factory, ReadInt32(CurrentFrame));
        });
    }

    private void AdvanceFactoryProductionCore(uint factory, int currentFrame)
    {
        WriteInt32(factory + FactoryProductionValueOffset, 53);
        WriteBytes(factory + FactoryProductionChangedOffset, [0]);
        WriteInt32(factory + FactoryProductionTimerStartOffset, currentFrame - 1);
